...
function synchronize_data() {
  ...
  async.series([
    ...
    // icinga-director does not use testing ids info at all
    //function(callback) {
  ...
  for(var item in data) {
    ...
    if(typeof(data[item].manager) === 'object') {  // multiple managers
      for(var j in data[item].manager) {
        out = "(" + num + ", '" + data[item].dn + "', '" + data[item].cn + "', '" + data[item].eduroamMemberType +  "', '" + data[item].labeledUri + "', '" + data[item].manager[j] + "')";
      ...
      out = "(" + num + ", '" + data[item].dn + "', '" + data[item].cn + "', '" + data[item].eduroamMemberType + "', '"  + data[item].labeledUri + "', '" + data[item].manager + "')";
  ...
  for(var item in data) {
    ...
    if(data[item].mail) {               // mail is defined
      if(typeof(data[item].mail) === 'object')
        out = "('" + data[item].dn + "', '" + data[item]['cn'] + "', '" + data[item].mail[0] + "', '" + data[item].uid + "')";  // first mail
      else
        out = "('" + data[item].dn + "', '" + data[item]['cn'] + "', '" + data[item].mail + "', '" + data[item].uid + "')";  // the only mail
    }
    else {
      out = "('" + data[item].dn + "', '" + data[item]['cn'] + "', '', '" + data[item].uid + "')";  // no mail available, set it to empty string
    }
...
function create_db_structure(callback)
{
  ...
  console.log("CREATE TABLE IF NOT EXISTS realm (id INT NOT NULL AUTO_INCREMENT, realm_dn VARCHAR(191) NOT NULL, realm_cn VARCHAR(512) NOT NULL, member_type VARCHAR(191) NOT NULL, xml_url VARCHAR(191) NOT NULL, realm_manager VARCHAR(191) NOT NULL, FOREIGN KEY (realm_manager) REFERENCES admin(admin_dn), PRIMARY KEY ( id ), UNIQUE ( id ), INDEX realm_idx (realm_dn));");
